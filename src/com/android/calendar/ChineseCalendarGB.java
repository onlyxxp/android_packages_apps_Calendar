...
public class ChineseCalendarGB {
    ...
    private int mGregorianDate;
    private boolean isGregorianLeap;
    private int dayOfYear;
    private int dayOfWeek; // 周日一星期的第一天
    private int chineseYear;
    private int mChineseMonth; // 负数表示闰月
    private int mChineseDate;
    ...
    public static void main(String[] arg) {
        ...
        System.out.println("xxxx main " + (System.currentTimeMillis() - start));
        c.computeSolarTerms();
    ...
    public int computeChineseFields() {
        ...
        mChineseMonth = baseChineseMonth;
        mChineseDate = baseChineseDate;
        // 第二个对应日，用以提高计算效率
        ...
        daysDiff += mGregorianDate - startDate;

        mChineseDate += daysDiff;
        int lastDate = daysInChineseMonth(chineseYear, mChineseMonth);
        int nextMonth = nextChineseMonth(chineseYear, mChineseMonth);
        while (mChineseDate > lastDate) {
            if (Math.abs(nextMonth) < Math.abs(mChineseMonth))
                chineseYear++;
            mChineseMonth = nextMonth;
            mChineseDate -= lastDate;
            lastDate = daysInChineseMonth(chineseYear, mChineseMonth);
            nextMonth = nextChineseMonth(chineseYear, mChineseMonth);
        }
        return 0;
    }
    ...
    private static LruCache<String, ArrayList<String>> sInfoCache = new LruCache<>(
            1000);

    public static ArrayList<String> getInfoByYMD(int year, int month, int day) {
        long start = System.currentTimeMillis();
        ...
        ArrayList<String> cachedInfo = sInfoCache.get(getKey(year, month, day));
        if (cachedInfo != null) {
            return cachedInfo;
        }

        ChineseCalendarGB calendarGB = new ChineseCalendarGB();
        calendarGB.setGregorian(year, month, day);
        calendarGB.computeChineseFields();
        calendarGB.computeSolarTerms();

        int chineseDay = calendarGB.mChineseDate;
        int chineseMonth = calendarGB.mChineseMonth;
        if (chineseDay == 1 && chineseMonth > 0) {
            // month in first day
            infos.add(sChineseMonthNames[chineseMonth - 1] + "月");
        } else {
            // day
            infos.add(sChineseDayNameStrings[chineseDay - 1]);
        }
        ...
        // Term
        if (calendarGB.mGregorianDate == calendarGB.mSectionalTerm) {
            infos.add(sSectionalTermNames[calendarGB.gregorianMonth - 1]);
        }
        if (calendarGB.mGregorianDate == calendarGB.mPrincipleTerm) {
            infos.add(sPrincipleTermNames[calendarGB.gregorianMonth - 1]);
        }
        System.out.println("xxx getInfoByYMD " + year + " " + month + " " + day
                + "   cost " + (System.currentTimeMillis() - start));
        sInfoCache.put(getKey(year, month, day), infos);
        return infos;
    }
    ...
    public String toString() {
        ...
        buf.append("Gregorian Date: " + mGregorianDate + "\n");
        ...
        buf.append("Chinese Month: " + mChineseMonth + "\n");
        buf.append("Chinese Date: " + mChineseDate + "\n");
        buf.append("Sectional Term: " + mSectionalTerm + "\n");
        buf.append("Principle Term: " + mPrincipleTerm + "\n");
        return buf.toString();
    }
    ...
    public String[] getMonthTable() {
        ...
        for (int i = mGregorianDate; i <= days; i++) {
            line += getDateString() + ' ';
            rollUpOneDay();
            if (dayOfWeek == 1) {
                table[wk] = line;
    ...
    private static String[] sChineseDayNameStrings = { "初一", "初二", "初三", "初四",
            "初五", "初六", "初七", "初八", "初九", "初十", "十一", "十二", "十三", "十四", "十五",
            "十六", "十七", "十八", "十九", "二十", "廿一", "廿二", "廿三", "廿四", "廿五", "廿六",
            "廿七", "廿八", "廿九", "三十", "三十一" };

    private static String[] sChineseMonthNames = { "正", "二", "三", "四", "五",
            "六", "七", "八", "九", "十", "冬", "腊" };
    private static String[] sPrincipleTermNames = { "大寒", "雨水", "春分", "谷雨",
            "夏满", "夏至", "大暑", "处暑", "秋分", "霜降", "小雪", "冬至" };
    private static String[] sSectionalTermNames = { "小寒", "立春", "惊蛰", "清明",
    ...
    public String getDateString() {
        ...
        if (gm.length() == 1)
            gm = ' ' + gm;
        String cm = String.valueOf(Math.abs(mChineseMonth));
        if (cm.length() == 1)
            cm = ' ' + cm;
        String gd = String.valueOf(mGregorianDate);
        if (gd.length() == 1)
            gd = ' ' + gd;
        String cd = String.valueOf(mChineseDate);
        if (cd.length() == 1)
            cd = ' ' + cd;
        if (mGregorianDate == mSectionalTerm) {
            str = " " + sSectionalTermNames[gregorianMonth - 1];
        } else if (mGregorianDate == mPrincipleTerm) {
            str = " " + sPrincipleTermNames[gregorianMonth - 1];
        } else if (mChineseDate == 1 && mChineseMonth > 0) {
            str = " " + sChineseMonthNames[mChineseMonth - 1] + "月";
        } else if (mChineseDate == 1 && mChineseMonth < 0) {
            str = "*" + sChineseMonthNames[-mChineseMonth - 1] + "月";
        } else {
            str = gd + '/' + cd;
    ...
    public int rollUpOneDay() {
        ...
        mChineseDate++;
        days = daysInChineseMonth(chineseYear, mChineseMonth);
        if (mChineseDate > days) {
            mChineseDate = 1;
            mChineseMonth = nextChineseMonth(chineseYear, mChineseMonth);
            if (mChineseMonth == 1)
                chineseYear++;
        }
        return 0;
